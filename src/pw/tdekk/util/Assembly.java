...
public class Assembly {
    ...
    public static List<MethodNode> findMethods(Filter<MethodNode> filter, Map<String, ClassNode> classes) {
        ...
        for (ClassNode cn : classes.values()) {
            methods.addAll(cn.methods.stream().filter(mn -> filter.accept(mn)).collect(Collectors.toList()));
        }
        return methods;
    }
    ...
    public static <E extends AbstractInsnNode> E getPrevious(AbstractInsnNode ain, Filter<AbstractInsnNode> filter,
                                                          int jumpDist) {
        if (ain == null) {
            return null;
        }
        int jump = 0;
        while ((ain = ain.getPrevious()) != null && (jumpDist == -1 || jump++ <= jumpDist)) {
    ...
    public static <E extends AbstractInsnNode> E getPrevious(AbstractInsnNode ain, Filter<AbstractInsnNode> filter) {
        return getPrevious(ain, filter, 15);
    }

    public static <E extends AbstractInsnNode> E getPrevious(AbstractInsnNode ain, int getOpcode, int jumpDist) {
        return getPrevious(ain, insn -> insn.getOpcode() == getOpcode, jumpDist);
    }

    public static <E extends AbstractInsnNode> E getPrevious(AbstractInsnNode ain, int getOpcode) {
        return getPrevious(ain, getOpcode, 15);
    }

    @SuppressWarnings("unchecked")
    public static <E extends AbstractInsnNode> E getNext(AbstractInsnNode ain, Filter<AbstractInsnNode> filter,
                                                      int jumpDist) {
        if (ain == null) {
            return null;
        }
        int jump = 0;
        while ((ain = ain.getNext()) != null && (jumpDist == -1 || jump++ <= jumpDist)) {
            if (filter.accept(ain)) {
                return (E) ain;
            }
        }
    ...
    public static int count(int getOpcode, MethodNode mn) {
        int count = 0;
        for (AbstractInsnNode ain : mn.instructions.toArray())
            count += (ain.getOpcode() == getOpcode ? 1 : 0);
        return count;
    }

    public static <E extends AbstractInsnNode> E getNext(AbstractInsnNode ain, Filter<AbstractInsnNode> filter) {
        return getNext(ain, filter, 15);
    }

    public static <E extends AbstractInsnNode> E getNext(AbstractInsnNode ain, int getOpcode, int jumpDist) {
        return getNext(ain, insn -> insn.getOpcode() == getOpcode, jumpDist);
    }

    public static <E extends AbstractInsnNode> E getNext(AbstractInsnNode ain, int getOpcode) {
        return getNext(ain, getOpcode, 15);
    }
            ...
            for (MethodNode mn : node.methods) {
                for (AbstractInsnNode ain : mn.instructions.toArray()) {
                    if (ain instanceof FieldInsnNode) {
                        ...
                        if (fin.owner.equals(fn.parent.name) && fin.name.equals(fn.name)) {
                            fin.name = newName;
                        }
                    }
                }
