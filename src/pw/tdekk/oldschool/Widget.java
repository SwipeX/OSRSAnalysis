...
@VisitorInfo(hooks = {"owner", "children", "x", "y", "width", "height", "itemId", "itemAmount",
        "id", "contentType", "type", "itemIds", "stackSizes", "scrollX", "scrollY", "textureId",
        "index", "text", "ownerId", "actions", "hidden", "boundsIndex", "mouseMovementListeners",
        "fontId", "repaintTick"})
public class Widget extends GraphVisitor {
    ...
    @Override
    public void visit() {
        add("owner", cn.getField(null, "L" + cn.name + ";"));
        add("children", cn.getField(null, "[L" + cn.name + ";"));
        visitAll(new PositionHooks());
        visitAll(new SizeHooks());
        visitAll(new TradeHooks());
        visitAll(new IdHooks());
        visitAll(new ContentType());
        visitAll(new Type());
        ...
        visitAll(new MouseMovementListeners());
        visitAll(new FontId());
    }
    ...
    private class MouseMovementListeners extends MethodVisitor {
        ...
        public void visitIntInsn(IntInsnNode iin) {
            ...
            if (iin.operand == 1412) {
                FieldInsnNode fin = Assembly.next(iin, ain -> {
                    if (ain.opcode() != PUTFIELD) {
                        return false;
                    }
                    FieldInsnNode f = (FieldInsnNode) ain;
                    return f.owner.equals(cn.name) && f.desc.equals("[Ljava/lang/Object;");
                });
                if (fin != null) {
                    addHook(new FieldHook("mouseMovementListeners", fin));
                    lock.set(true);
                }
            }
        }
    ...
    private class FontId extends BlockVisitor {
        ...
        public void visit(Block block) {
            block.tree().accept(new NodeVisitor() {
                public void visitMethod(MethodMemberNode mmn) {
                    if (mmn.opcode() == INVOKEVIRTUAL && mmn.desc().contains("J") &&
                            mmn.first(GETSTATIC) != null) {
                        TypeNode tn = mmn.nextType();
                        if (tn != null && !tn.type().equals(clazz("WidgetNode"))) {
                            FieldMemberNode fmn = (FieldMemberNode) mmn.layer(I2L, IMUL, GETFIELD);
                            if (fmn != null && fmn.owner().equals(cn.name) && fmn.first(ALOAD) != null) {
                                addHook(new FieldHook("fontId", fmn.fin()));
                                lock.set(true);
                            }
                        }
                    }
    ...
    protected static class WidgetRepaintTick extends BlockVisitor {

        private String cycleKey;
        private GraphVisitor gv;

        public WidgetRepaintTick(GraphVisitor gv, String cycleKey) {
            this.gv = gv;
        ...
        public void visit(Block block) {
            block.tree().accept(new NodeVisitor() {
                public void visitField(FieldMemberNode fmn) {
                    if (fmn.opcode() == PUTFIELD && fmn.owner().equals(gv.cn.name) && fmn.desc().equals("I")) {
                        FieldMemberNode gameCycle = (FieldMemberNode) fmn.layer(IMUL, GETSTATIC);
                        if (gameCycle != null && gameCycle.key().equals(cycleKey)) {
                            gv.addHook(new FieldHook("repaintTick", fmn.fin()));
                            lock.set(true);
                        }
                    }
                }
