...
public class AbstractNode extends Tree<AbstractNode> implements Opcodes {
        ...
        switch (insn.type()) {
            case INSN: {
                if (opcode() >= ICONST_M1 && opcode() <= DCONST_1) {
                    nv.visitNumber((org.objectweb.asm.commons.cfg.tree.node.NumberNode) this);
            ...
            case TYPE_INSN: {
                nv.visitType((org.objectweb.asm.commons.cfg.tree.node.TypeNode) this);
                break;
            }
            case FIELD_INSN: {
            ...
            case JUMP_INSN: {
                nv.visitJump((org.objectweb.asm.commons.cfg.tree.node.JumpNode) this);
                break;
            }
            case LABEL: {
                nv.visitLabel(this);
    ...
    public org.objectweb.asm.commons.cfg.tree.node.NumberNode firstNumber() {
        return first(org.objectweb.asm.commons.cfg.tree.node.NumberNode.class);
    }
    ...
    public org.objectweb.asm.commons.cfg.tree.node.TypeNode firstType() {
        return first(org.objectweb.asm.commons.cfg.tree.node.TypeNode.class);
    }

    public org.objectweb.asm.commons.cfg.tree.node.JumpNode firstJump() {
        return first(org.objectweb.asm.commons.cfg.tree.node.JumpNode.class);
    }
    ...
    public org.objectweb.asm.commons.cfg.tree.node.NumberNode nextNumber() {
    ...
    public MethodMemberNode nextMethod() {
        return nextMethod(1);
    }

    public org.objectweb.asm.commons.cfg.tree.node.JumpNode nextJump(int max) {
        return next(org.objectweb.asm.commons.cfg.tree.node.JumpNode.class, max);
    }
    ...
    public org.objectweb.asm.commons.cfg.tree.node.TypeNode nextType(int max) {
        return next(org.objectweb.asm.commons.cfg.tree.node.TypeNode.class, max);
    }
    ...
    public List<AbstractNode> findChildren(int opcode) {
        List<AbstractNode> children = this.stream().filter(n -> n.opcode() == opcode).collect(Collectors.toList());
